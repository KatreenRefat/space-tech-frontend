...
// /**
//  * صفحة إنشاء حساب جديد - صلّحلي
//  * ------------------------------------------------------
//  * الشاشة دي هي "استكمال البيانات" اللي بتيجي بعد تسجيل الدخول بالـ OTP
//  * (يعني المستخدم لازم يكون معاه accessToken في localStorage قبل ما يوصل هنا).
//  *
//  * متربطة بـ: POST /api/v1/me/signup  (multipart/form-data)
//  *
//  * الفورم ديناميكي:
//  *  - كل المستخدمين: الاسم، المدينة، مكانك (= address)، دورك (عميل/فني)
//  *  - لو اختار "فني": مجال الخدمة، الرقم القومي، صورة شخصية، فيش وتشبيه
//  *  - بعد الإرسال:
//  *      فني  -> شاشة انتظار "معلش استنا و هنرد عليك بكرا" (accountState = WAITING_FOR_APPROVAL)
//  *      عميل -> TODO: حددي بعدين توديه فين (الحساب بيبقى READY على طول)
//  *
//  * ============================================================
//  * افتراضات اتخذتها (اتأكدي منها):
//  * ============================================================
//  * 1) categoryId: GET /api/v1/public/categories لسه بيرجع 501 (مش شغال).
//  *    بنحاول نجيبها، ولو فشلت بنستخدم قايمة احتياطية وبنبعت categoryId="1"
//  *    (المضمون إنه موجود في الـ seed data). دور على FALLBACK_CATEGORY_ID تحت.
//  * 2) latitude/longitude: بنجيبهم تلقائيًا من Geolocation المتصفح. لو المستخدم
//  *    رفض الإذن، بنوريله رسالة يفعّل الصلاحية بدل ما نبعت قيمة وهمية.
//  * 3) الحساب بيتفعل مباشرة للعميل (READY) - مفيش شاشة "بعد النجاح" محددة
//  *    للعميل في التصميم، فحاطة مكانها TODO واضح في handleSubmit.
//  */

// const API_BASE = "";// فاضي عشان نستخدم الـ proxy بتاع Vite بدل الرابط المباشر
...
// // TODO: لما GET /api/v1/public/categories يشتغل فعليًا (مش 501)،
// // الأسماء والـ IDs هتيجي من هناك تلقائيًا وممكن تشيلي السطر ده.
...
//   categoryId: "",
//   nationalId: "",
//   personalPhoto: null, // -> profileImage
//   criminalRecord: null, // -> criminalRecordFile
//   agree: false,
// };
...
// // بيحول رسائل الأخطاء الراجعة من details[] لشكل { fieldName: message }
// function mapApiErrors(details) {
//   const map = {};
//   (details || []).forEach((d) => {
//     // الباك اند بيرجع أسماء زي address/latitude/longitude، وإحنا عندنا
//     // حقل واحد اسمه location بيغطيهم، فبنجمعهم كلهم في نفس رسالة الخطأ
//     const key =
...
// /* اللوجو */
// /* ---------------------------------------------------------- */
// function Logo({ size = "h-16 w-16", text = "text-xl" }) {
//   return (
//     <div className="flex flex-col items-center">
//       <svg className={size} viewBox="0 0 64 64" fill="none" aria-hidden="true">
//         <path
//           d="M8 32 32 12l24 20"
...
//         />
//         <path
//           d="M14 28v22a2 2 0 0 0 2 2h32a2 2 0 0 0 2-2V28"
//           stroke="var(--color-primary-500)"
//           strokeWidth="4"
//           strokeLinecap="round"
//           strokeLinejoin="round"
//         />
//         <path
//           d="M26 36a5 5 0 1 1 8 4l6 6-2 2-6-6a5 5 0 0 1-6-6Z"
//           fill="var(--color-secondary-500)"
//         />
//       </svg>
//       <span className={`mt-2 font-cairo font-semibold text-primary-500 ${text}`}>
//         صَلّْحِلِي
//       </span>
//     </div>
//   );
...
// /* ---------------------------------------------------------- */
// /* شاشة الانتظار بعد إرسال بيانات الفني */
// /* ---------------------------------------------------------- */
...
//   // حاول تجيب فئات الخدمة الحقيقية (لو الـ endpoint اشتغل)
//   useEffect(() => {
//     fetch(CATEGORIES_ENDPOINT, { headers: { ...getAuthHeader() } })
...
//       body.append("role", form.role); // CUSTOMER | TECHNICIAN (كابيتال زي ما الـ API عايز)
...
//         method: "POST",
//         headers: {
//           ...getAuthHeader(), // Content-Type متبعتش يدوي مع FormData - المتصفح بيحطها لوحده مع الـ boundary
//         },
//         body,
//       });
...
//       if (isTechnician) {
//         setSubmitted(true); // شاشة "معلش استنا وهنرد عليك بكرا"
//       } else {
//         // TODO: العميل يروح فين بعد التسجيل؟ (مثلاً: navigate("/home"))
//         // الحساب بقى READY فعليًا من ناحية السيرفر، بس التصميم مفيهوش
//         // شاشة محددة للعميل بعد النجاح - حطي هنا التوجيه اللي يناسبك.
//         setSubmitted(true);
//       }
//     } catch (err) {
...
//     >
//       {/* الدوائر والأيقونات - على الصفحة كلها */}
//       <div className="pointer-events-none absolute -top-16 -left-16 z-0 h-72 w-72 rounded-full bg-primary-500" />
//       <div className="pointer-events-none absolute -bottom-20 -right-20 z-0 h-80 w-80 rounded-full bg-primary-500" />
//       <WrenchIcon className="pointer-events-none absolute bottom-10 right-10 z-0 h-8 w-8 text-primary-500" />
//       <svg
//         className="pointer-events-none absolute top-20 left-20 z-0 h-8 w-8 text-secondary-500"
//         viewBox="0 0 24 24"
//         fill="currentColor"
//         aria-hidden="true"
//       >
//         <path d="M14 4 20 10l-2 2-1-1-7 7-3-3 7-7-1-1 2-2Z" />
//       </svg>
//       <svg
//         className="pointer-events-none absolute top-14 right-24 z-0 h-6 w-6 text-secondary-500"
//         viewBox="0 0 24 24"
//         fill="currentColor"
//         aria-hidden="true"
//       >
...
//                 <h1 className="text-2xl font-semibold text-primary-500">اعمل حساب جديد</h1>
//                 <svg
...
//                   dir="rtl"
//                 >
//                   محتاجين نعرف مكانك عشان نكمل التسجيل - من فضلك فعّلي صلاحية الموقع في
//                   المتصفح وارجعي حدّثي الصفحة.
//                 </p>
//               )}
...
//                 {errors.submit && (
//                   <p className="text-center text-xs text-secondary-700">{errors.submit}</p>
//                 )}
...
//                 </label>
//                 {errors.agree && (
//                   <p className="-mt-2 text-center text-xs text-secondary-700">{errors.agree}</p>
//                 )}
//               </form>
//             </div>
//           </div>
...
//             <div className="flex h-56 w-56 items-center justify-center rounded-full bg-white shadow-lg">
//               <Logo />
//             </div>
//           </div>
//         </>
...
//  * ============================================================
//  * - محتاجة الشاشة دي accessToken جاهز في localStorage (بعد تسجيل
//  *   الدخول بالـ OTP في شاشة تانية) عشان تنجح.
//  * - API_BASE فاضي عشان نستخدم Vite proxy بتاع /api أثناء التطوير
//  *   (شوفي vite.config.js). لو مفيش proxy عندك، حطي هنا الرابط كامل:
//  *   "https://back.markwasfy00.xyz"
//  * - categoryId: بنحاول نجيب القايمة الحقيقية من GET /categories،
//  *   ولو فشلت (لسه 501 من ناحية الباك اند) بنستخدم قايمة احتياطية
//  *   وبنبعت categoryId="1" (مضمون موجود في الـ seed data).
//  * - latitude/longitude: بنجيبهم تلقائيًا من Geolocation المتصفح.
//  * ============================================================ */
// const API_BASE = "";// فاضي عشان نستخدم الـ proxy بتاع Vite بدل الرابط المباشر
// const SIGNUP_ENDPOINT = `${API_BASE}/api/v1/me/signup`;
// const CATEGORIES_ENDPOINT = `${API_BASE}/api/v1/public/categories`;
...
//   // حاول تجيب فئات الخدمة الحقيقية (لو GET /categories اشتغل)
//   useEffect(() => {
//     fetch(CATEGORIES_ENDPOINT, { headers: { ...getAuthHeader() } })
//       .then((res) => (res.ok ? res.json() : Promise.reject()))
//       .then((json) => {
